/*********************************************************************************************************************** 
                                          PRIVATE INVESTOR'S MONTHLY ACTIVITY REPORT 
***********************************************************************************************************************/
SELECT
...
NULL /*TO_CHAR(SCM7.ACDELIVERYDATE,'MM/DD/YYYY')*/                              AS DateSold,                            --AZ: UPDATED LOGIC SINCE HISTORICAL BALANCES WERE NOT LOADED IN CHIP
...
NULL /*MAS3.HUDUPB*/                                                            AS initial_loan_balance,                --AZ: UPDATED LOGIC SINCE HISTORICAL BALANCES WERE NOT LOADED IN CHIP
NULL /*RLSM.SCHD_PMNT_BAL*/                                                     AS IDA_schedule_advances,               --AZ: UPDATED LOGIC SINCE HISTORICAL BALANCES WERE NOT LOADED IN CHIP
NULL /*RLSM.UNSCHD_PMNT_BAL*/                                                   AS IDA_unscheduled_advances,            --AZ: UPDATED LOGIC SINCE HISTORICAL BALANCES WERE NOT LOADED IN CHIP
NULL /*RLSM.PREPMNT_BAL*/                                                       AS IDA_prepayments,                     --AZ: UPDATED LOGIC SINCE HISTORICAL BALANCES WERE NOT LOADED IN CHIP
NULL /*RLSM.SERV_FEE_BAL*/                                                      AS IDA_service_fee,                     --AZ: UPDATED LOGIC SINCE HISTORICAL BALANCES WERE NOT LOADED IN CHIP
NULL /*RLSM.MIP_BAL*/                                                           AS IDA_MIP_interest_accrued,            --AZ: UPDATED LOGIC SINCE HISTORICAL BALANCES WERE NOT LOADED IN CHIP
NULL /*RLSM.INTEREST_BAL*/                                                      AS IDA_contra_Interest_accrued,         --AZ: UPDATED LOGIC SINCE HISTORICAL BALANCES WERE NOT LOADED IN CHIP
NULL/*RLSM.PAYOFF_FEE_BAL*/                                                     AS IDA_payoff_fees,                     --AZ: UPDATED LOGIC SINCE HISTORICAL BALANCES WERE NOT LOADED IN CHIP
NULL/*RLSM.PAYOFF_BAL*/                                                         AS IDA_payoff_amount,                   --AZ: UPDATED LOGIC SINCE HISTORICAL BALANCES WERE NOT LOADED IN CHIP
NULL/*RLSM.PAYOFF_REFUND*/                                                      AS IDA_Excess_Shortfall,                --AZ: UPDATED LOGIC SINCE HISTORICAL BALANCES WERE NOT LOADED IN CHIP
...
NULL /*RLSM.LEN_OF_TERM*/                                                       AS months_remaining                     --AZ: UPDATED LOGIC SINCE DATA NOT BEING LOADED CORRECTLY IN CHIP
FROM SCM7_CD     SCM7
...
LEFT OUTER JOIN(SELECT RLSM.LOAN
                      ,RLSM.LOAN_BALANCE AS END_BALANCE
                FROM REVLOANSERVICINGMONTH  RLSM 
                WHERE RLSM.RECORD_DATE = (SELECT TO_DATE(LAST_DAY(ADD_MONTHS(?,-1)),'DD-MON-YY')+1 AS CurrentMonthFirstDay FROM dual)
                ) CURMONTHBALANCE ON SCM7.LOAN = CURMONTHBALANCE.LOAN
LEFT OUTER JOIN(SELECT RLP.LOAN
                      ,MAX(RLP.POST_DATE) AS LAST_DRAW_DT
                FROM REVLOANPAYMENTS RLP
                WHERE RLP.POST_DATE BETWEEN ? AND ?
