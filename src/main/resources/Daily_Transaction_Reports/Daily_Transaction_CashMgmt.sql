/*********************************************************************************************************************** 
                                        CASH MANAGEMENT DAILY TRANSACTIONS REPORT
***********************************************************************************************************************/
SELECT   
...
(CASE WHEN RLPS.STATUS = '0' THEN 'Cancelled'
      WHEN RLPS.STATUS = '1' THEN 'Active'
      ELSE ''  
END)                                                                               AS tran_status,                      --UPDATED LOGIC
--RLPS.STATUS                                                                        AS status,                         --DELETED THIS FIELD
(CASE WHEN RLPS.STATUS = '0' THEN TO_CHAR(RLPS.UPDATED_ON,'MM/DD/YYYY')
      ELSE '' 
 END)                                                                              AS tran_status_changed_date,         --UPDATED LOGIC TO CAPTURED CANCELLED TRANS
(CASE WHEN RLPS.STATUS = '0'
      AND TO_CHAR(RLPS.POST_DATE, 'MM/DD/YYYY') = TO_CHAR(RLPS.UPDATED_ON, 'MM/DD/YYYY')
      THEN 'Y'
      ELSE 'N'
 END)                                                                              AS Same_Day_Tran_Cancellation,       --ADDED FIELD AS PER PBI# 54072
RLPS.LOAN_BALANCE                                                                  AS UPB,                              --ADDED FIELD AS PER PBI# 54072
...
NULL                                                                               AS writeoff_payoff_cost_center,      --CHANGED NAME FROM writeoff_costcenter_assignment TO writeoff_payoff_cost_center
NULL                                                                               AS Writeoff_Adjs_cost_center,        --ADDED IT AS PER PBI# 31842
BRW1.FIRST_NAME                                                                    AS Borr_First_Name,                  --UPDATED FIELD NAME FROM first_name TO Borr_First_Name
BRW1.LAST_NAME                                                                     AS Borr_Last_Name,                   --UPDATED FIELD NAME FROM last_name TO Borr_Last_Name
BRW2.FIRST_NAME                                                                    AS CoBorr_First_Name,                --ADDED IT AS PER PBI# 54072
BRW2.LAST_NAME                                                                     AS CoBorr_Last_Name,                 --ADDED IT AS PER PBI# 54072MAS3.ACINVESTORCODE                                                                AS investor_code,
MAS3.ACINVESTORCODE                                                                AS investor_code,
SCM7.THEIR_LOAN_NO                                                                 AS investor_loan_no,                 --UPDATED LOGIC
'Payment'                                                                          AS payment_type,   
RLPS.RECORD_TYPE                                                                   AS RECORD_TYPE,   
RLPS.USER_ID                                                                       AS user_id,  
(CASE WHEN RLPS.PMNT_METHOD = 'MC' THEN 'Mail Check'
      WHEN RLPS.PMNT_METHOD = 'DD' THEN 'Direct Deposit'
      WHEN RLPS.PMNT_METHOD = 'AC' THEN 'Access Check'
      ELSE ''
 END)                                                                              AS pmnt_method,  
...
RLPS.NOTE                                                                          AS note                               --ADDED FIELD AS PER PBI# 54072
FROM REVLOANPAYMENTS   RLPS
INNER JOIN (SELECT LOAN, THEIR_LOAN_NO
            FROM SCM7_CD        SCM7
...
LEFT OUTER JOIN (SELECT BRW1.LOAN,BRW1.FIRST_NAME,BRW1.LAST_NAME
                 FROM BRW1_CD        BRW1
                 INNER JOIN (SELECT LOAN,BRWR_ID_2
                             FROM   MAS1_CD  MAS1
                             WHERE  MAS1.HISTORY = 'L' AND MAS1.ITEM = 'zzz' AND MAS1.STATUS = '1'
                             ) MAS1 ON BRW1.LOAN = MAS1.LOAN AND BRW1.BRWR_NO = MAS1.BRWR_ID_2
                 WHERE BRW1.HISTORY = 'L' AND BRW1.ITEM = 'zzz'	AND BRW1.STATUS = '1'
                 ) BRW2 ON RLPS.LOAN = BRW2.LOAN
...
WHERE RLPS.HISTORY = 'L' AND  RLPS.ITEM = 'zzz' AND (RLPS.STATUS IN ('1','0') OR (RLPS.STATUS = '0' AND RLPS.MOD_DATE IS NOT NULL))  
...
CASE WHEN RLPPS.PREPAY_TYPE = '31'   THEN '31 - Payoff Death'
     ...
     WHEN RLPPS.PREPAY_TYPE = '80'   AND RLPPS.ACTION_CODE = '80' THEN '80 - Part Pre-Payment-Reduce UPB only'          --ADDED MAPPING AS PER PBI# 54072
     ...
     WHEN RLPPS.PREPAY_TYPE = '80'   AND RLPPS.ACTION_CODE = '80A' THEN '80A - Part Pre-Payment-Short Sale Proceeds'    --ADDED MAPPING AS PER PBI# 54072
     WHEN RLPPS.PREPAY_TYPE = '80'   AND RLPPS.ACTION_CODE = '80B' THEN '80B - Part Pre-Payment-Foreclosure Sale'       --ADDED MAPPING AS PER PBI# 54072
     WHEN RLPPS.PREPAY_TYPE = '80'   AND RLPPS.ACTION_CODE = '80C' THEN '80C - Part Pre-Payment-REO'                    --ADDED MAPPING AS PER PBI# 54072
     WHEN RLPPS.PREPAY_TYPE = '80'   AND RLPPS.ACTION_CODE = '80D' THEN '80D - Part Pre-Payment-HUD Claim'              --ADDED MAPPING AS PER PBI# 54072
     WHEN RLPPS.PREPAY_TYPE = '80'   AND RLPPS.ACTION_CODE = '80E' THEN '80E - Part Pre-Payment-Supplemental Claim'     --ADDED MAPPING AS PER PBI# 54072
...
(CASE WHEN RLPPS.STATUS = '0'        THEN 'Cancelled'
      WHEN RLPPS.STATUS IN ('1','5') THEN 'Active'
      ELSE ''  
END)                                                                              AS tran_status,                       --UPDATED LOGIC
--RLPPS.STATUS                                                                        AS status,                        --DELETED THIS FIELD
(CASE WHEN RLPPS.STATUS = '0' THEN TO_CHAR(RLPPS.UPDATED_ON,'MM/DD/YYYY')
     ELSE '' 
 END)                                                                             AS tran_status_changed_date,          --No Req to Updated logic to account for Cancelled transactions
(CASE WHEN RLPPS.STATUS = '0'
      AND TO_CHAR(RLPPS.POST_DATE, 'MM/DD/YYYY') = TO_CHAR(RLPPS.UPDATED_ON, 'MM/DD/YYYY')
      THEN 'Y'
      ELSE 'N'
 END)                                                                             AS Same_Day_Tran_cancellation,        --ADDED FIELD AS PER PBI# 54072
RLPPS.LOAN_BALANCE                                                                AS UPB,                               --ADDED FIELD AS PER PBI# 54072
...
BRW1.LAST_NAME                                                                    AS Borr_Last_Name,                    --UPDATED FIELD NAME FROM last_name TO Borr_Last_Name
BRW2.FIRST_NAME                                                                   AS CoBorr_First_Name,                 --ADDED IT AS PER PBI# 54072
BRW2.LAST_NAME                                                                    AS CoBorr_Last_Name,                  --ADDED IT AS PER PBI# 54072
MAS3.ACINVESTORCODE                                                               AS investor_code,
SCM7.THEIR_LOAN_NO                                                                AS investor_loan_no,                  --UPDATED LOGIC
...
NULL                                                                              AS mailto_zip,
RLPPS.NOTE                                                                        AS note                               --ADDED FIELD AS PER PBI# 54072
...
CASE WHEN RLCA.CORP_ADV_TYPE = 'PRPTAX' THEN 'PRPTAX - Property Tax'
     ...
     WHEN RLCA.CORP_ADV_TYPE = 'NEGLOC' THEN 'NEGLOC - Negative LOC Payment'
...
(CASE WHEN RLCA.STATUS = '0' THEN 'Cancelled'
      WHEN RLCA.STATUS = '1' THEN 'Active'
      ELSE ''  
END)                                                                              AS tran_status,                       --UPDATED LOGIC TO ACCOUNT FOR CANCELLED TRANSACTIONS
--RLCA.STATUS                                                                        AS status,                         --DELETED THIS FIELD
(CASE WHEN RLCA.STATUS = '0' THEN TO_CHAR(RLCA.UPDATED_ON,'MM/DD/YYYY')
     ELSE '' 
 END)                                                                             AS tran_status_changed_date,          --Updated logic to account for Cancelled transactions
(CASE WHEN RLCA.STATUS = '0'
      AND TO_CHAR(RLCA.POST_DATE, 'MM/DD/YYYY') = TO_CHAR(RLCA.UPDATED_ON, 'MM/DD/YYYY')
      THEN 'Y'
      ELSE 'N'
 END)                                                                             AS Same_Day_Tran_cancellation,
RLS.LOAN_BALANCE                                                                  AS UPB,                               --ADDED FIELD AS PER PBI# 54072
...
'Mail Check'                                                                      AS pmnt_method,                       --UPDATED LOGIC
NULL                                                                              AS deposit_bank_name,  
NULL                                                                              AS deposit_bank_acnt_no,  
NULL                                                                              AS deposit_bank_rout_no,  
NULL                                                                              AS mailto_street,  
...
NULL                                                                              AS mailto_state,  
NULL                                                                              AS mailto_zip,  
RLCA.NOTE                                                                         AS note                               --ADDED FIELD AS PER PBI# 54072
...
LEFT OUTER JOIN (SELECT BRW1.LOAN,BRW1.FIRST_NAME,BRW1.LAST_NAME
                 ...
                 WHERE BRW1.HISTORY = 'L' AND BRW1.ITEM = 'zzz'	AND BRW1.STATUS = '1'
                 ) BRW2 ON RLCA.LOAN = BRW2.LOAN
LEFT OUTER JOIN (SELECT LOAN, LOAN_BALANCE
                 FROM REVLOANSERVICING RLS
                 WHERE  RLS.HISTORY = 'L' AND RLS.ITEM = 'zzz' AND RLS.STATUS = '1'
                 ) RLS ON RLS.LOAN = RLCA.LOAN
LEFT OUTER JOIN (SELECT LOAN
                 FROM   MAS1_CD        MAS1
                 WHERE  MAS1.HISTORY = 'L' AND MAS1.ITEM = 'zzz' AND MAS1.STATUS = '1' AND MAS1.ACIDX = '01'
